...
drug_era_data <- function(){
    drug_era_sql<- splitSql(readSql(paste0(.libPaths()[1],"/gemini/data/extdata/Drug_era_query.sql")))
    if(connection@dbms == 'sql server'){
        drug_era_record_sql <- splitSql(readSql(paste0(.libPaths()[1],"/gemini/data/extdata/System_query.sql")))[1]
    }else if(connection@dbms == 'postgresql'){
        drug_era_record_sql <- splitSql(readSql(paste0(.libPaths()[1],"/gemini/data/extdata/System_query.sql")))[2]
    }
################################################################################

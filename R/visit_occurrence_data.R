...
visit_occurrence_data <- function(){
    visit_sql<- splitSql(readSql(paste0(.libPaths()[1],"/gemini/data/sql/Visit_occurrence_query.sql")))
    if(connection@dbms == 'sql server'){
        visit_record_sql <- splitSql(readSql(paste0(.libPaths()[1],"/gemini/data/sql/System_query.sql")))[1]
    }else if(connection@dbms == 'postgresql'){
        visit_record_sql <- splitSql(readSql(paste0(.libPaths()[1],"/gemini/data/sql/System_query.sql")))[2]
    }
################################################################################

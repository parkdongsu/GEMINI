...
person_data <- function(){
    person_sql<- splitSql(readSql(paste0(.libPaths()[1],"/gemini/data/sql/Person_query.sql")))
    if(connection@dbms == 'sql server'){
        person_record_sql <- splitSql(readSql(paste0(.libPaths()[1],"/gemini/data/sql/System_query.sql")))[1]
    }else if(connection@dbms == 'postgresql'){
        person_record_sql <- splitSql(readSql(paste0(.libPaths()[1],"/gemini/data/sql/System_query.sql")))[2]
    }
################################################################################

...
condition_occurrence_data <- function(){
    condition_sql<- splitSql(readSql(paste0(.libPaths()[1],"/gemini/data/sql/Condition_occurrence_query.sql")))
    if(connection@dbms == 'sql server'){
        condition_record_sql <- splitSql(readSql(paste0(.libPaths()[1],"/gemini/data/sql/System_query.sql")))[1]
    }else if(connection@dbms == 'postgresql'){
        condition_record_sql <- splitSql(readSql(paste0(.libPaths()[1],"/gemini/data/sql/System_query.sql")))[2]
    }
################################################################################
